...
func getPrediction(residualTest map[string][][][][]PixelGray, agreg [][]PixelGray, nameCameras []string) []float64 {
	...
	//	scores[i] = make(float64, len(residualTest[nameCameras[0]][0][0][0]))
	//}
	...
	for i := 0; i < len(nameCameras); i++ { // numCam = 4
		...
		/*if layer == "B" && j == 0 {
			for z := 0; z < len(residualTest[nameCameras[i]][j]); z++ { // numImg
				result = scalarProduct(residualTest[nameCameras[i]][0][z], agreg)
				//scores[z].pix = result
				scores = append(scores, result)
			}
		} else if layer == "G" && j == 1 {
			for z := 0; z < len(residualTest[nameCameras[i]][j]); z++ { // numImg
				result = scalarProduct(residualTest[nameCameras[i]][1][z], agreg)
				scores = append(scores, result)
			}
		} else if layer == "R" && j == 2 {
			for z := 0; z < len(residualTest[nameCameras[i]][j]); z++ { // numImg
				result = scalarProduct(residualTest[nameCameras[i]][2][z], agreg)
				scores = append(scores, result)
			}
		}*/
		//}
...
/*
	// PREDICTION (SCALAR PRODUCT: RESIDUAL AND PRNU)
