...
func getEncryptedPrediction(finalPrnu [][]PixelGray /*estimation user1: [rows][columns]*/, finalResiduals [][][]PixelGray /*[image][rows][columns]*/, N int /*numTestImages*/) ([]float64, []float64) {
	...
	elapsedEncryptCloud = elapsedEncryptParty * time.Duration(len(P))
	...
	elapsedEvalParty = elapsedEvalCloudCPU / time.Duration(len(P))
	...
	elapsedDecCloud = elapsedDecParty * time.Duration(len(P))
	...
	fmt.Printf("\tTOTAL TIME: %s\n", elapsedRKGCloud+elapsedCKGCloud+elapsedRTGCloud+
		elapsedEncryptCloud+elapsedEvalCloudCPU+elapsedPCKSCloud+elapsedDecCloud)
...
func getInputs(p []*party, residuals [][][]PixelGray, finalPrnus [][]PixelGray) (expRes []float64) {
	...
	// RESULTADOS PREDICCIÓN EN CLARO -> Num Parties - 1 x params.N() -> cada fila el mismo valor repetido
	expRes = make([]float64, len(p)-1)

	// Generate Prediction Expected Results
	for i, pi := range p[1:] { // desde p[1] hasta p[len(p) - 1], hay que comparar con p[0]
		for j := range pi.input {
			for k := range pi.input[j] {
				//expRes[i] += (pi.input[j][k] * p[0].input[j][k]) // producto escalar
